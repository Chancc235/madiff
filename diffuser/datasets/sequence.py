...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if self.use_action:
            if self.discrete_action and "legal_actions" in fields.keys:
                # For discrete actions, use the number of possible actions (from legal_actions)
                # instead of the scalar action index dimension
                self.action_dim = fields.legal_actions.shape[-1]
                print(f"Discrete action space detected: action_dim = {self.action_dim}")
            else:
                # For continuous actions, use the actual action dimension
...
class VAESequenceDataset(SequenceDataset):
    """
    Dataset for VAE-based diffusion RL
    Separates actions and observations for VAE encoding/decoding
    """
    ...
    def __getitem__(self, idx: int, agent_idx: Optional[int] = None):
        ...
        if self.use_action:
            ...
            actions = np.zeros((observations.shape[0], observations.shape[1], 0))

        # For VAE-based approach, we keep actions and observations separate
        # Full trajectory for loss computation (actions + observations)
        if self.use_action:
            full_trajectory = np.concatenate([actions, observations], axis=-1)
        else:
            full_trajectory = observations

        # Historical trajectory for conditioning (only history part)
        if self.history_horizon > 0:
            history_trajectory = full_trajectory[:self.history_horizon]
            
            # For discrete actions, convert scalar indices to one-hot for trajectory encoder
            if self.discrete_action and self.use_action and self.action_dim > 1:
        ...
        if hasattr(self.fields, 'rewards') and self.fields.rewards is not None:
            rewards = self.fields.rewards[path_ind, history_start:end]
        else:
            # Create dummy rewards if not available
            rewards = np.zeros((observations.shape[0], observations.shape[1]))
            
        # Extract dones if available  
        if hasattr(self.fields, 'terminals') and self.fields.terminals is not None:
            dones = self.fields.terminals[path_ind, history_start:end]
        else:
            # Create dummy dones (only last step is terminal)
            dones = np.zeros((observations.shape[0], observations.shape[1]), dtype=bool)
            if end >= self.fields.normed_observations.shape[1]:
                dones[-1] = True  # Mark last step as terminal

        batch = {
            "x": full_trajectory,  # Full trajectory [actions, observations]
